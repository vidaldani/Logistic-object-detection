...
class YoloSAM2FilterSegmentationNode(Node):

    def __init__(self):
        super().__init__('yolo_sam2_filter_segmentation_node')
        
        # Initialize SAM Model for segmentation
        # self.sam_model = sam_model_registry["vit_b"](checkpoint="/home/user/models/sam_vit_b_01ec64.pth")
        # self.sam_model = sam_model_registry["vit_l"](checkpoint="/home/user/models/sam_vit_l_0b3195.pth")
        # self.sam_model = sam_model_registry["vit_h"](checkpoint="/home/user/models/sam_vit_h_4b8939.pth")

        # self.predictor = SamPredictor(self.sam_model)
         
        # checkpoint = "/home/user/segment-anything-2/checkpoints/sam2_hiera_tiny.pt"
        # model_cfg = "/home/user/segment-anything-2/sam2_configs/sam2_hiera_t.yaml"  # The configuration for SAM 2 Tiny
        checkpoint = "/home/tumwfml-ubunt6/SAM_models/segment-anything-2/checkpoints/sam2_hiera_tiny.pt"
        model_cfg = "sam2_hiera_t.yaml"  # The configuration for SAM 2 Tiny
        ...
        self.get_logger().info("YoloSAM2FilterSegmentationNode initialized.")
    ...
    def inference_callback(self, msg):
        ...
        if self.bounding_boxes and self.depth_image is not None and self.rgb_image is not None:
            ...
            for box in self.bounding_boxes:
                ...
                # Get all non-zero points in the filtered image
                # non_zero_coords = np.column_stack(np.where(filtered_image > 0))

                # Ensure we have non-zero points to use for the seed
                # if len(non_zero_coords) > 0:
                    # Choose the median point in the filtered image as the seed point
                    # seed_point = tuple(non_zero_coords[len(non_zero_coords) // 2])
                # else:
                    # If no non-zero points are found, default to the center of the image
                    # cropped_height, cropped_width = filtered_image.shape
                    # seed_point = (cropped_width // 2, cropped_height // 2)
                ...
                # Calculate the center of the cropped image for the region growing seed point
                # cropped_height, cropped_width = cropped_depth_image.shape
                # center_x = cropped_width // 2
                # center_y = cropped_height // 2
                # region_growing_filtered_image = self.apply_region_growing_filter(filtered_image, seed_point=seed_point, threshold=0.05)
                ...
                # self.publish_filtered_image(region_growing_filtered_image, "/filtered_depth_image")
    ...
    def apply_region_growing_filter(self, depth_image, seed_point, threshold=0.07):
        """
        Apply region growing algorithm to filter the depth image.
        
        Parameters:
        - depth_image: The depth image (numpy array).
        - seed_point: Optional tuple (x, y) that acts as the seed point for region growing. If not provided, the center of the image will be used.
        - threshold: Threshold for the similarity of neighboring pixel depths (default is 0.05).
        
        Returns:
        - Filtered depth image (numpy array) with only the region grown from the seed point.
        """
        
        # Ensure the depth image is valid
        ...
        # Set seed point to the center of the depth image if not provided
        # if seed_point is None:
        #     seed_point = (width // 2, height // 2)

        # Get the depth value at the seed point
        seed_depth_value = depth_image[seed_point[1], seed_point[0]]

        # Ensure seed depth value is valid (i.e., non-zero)
        if seed_depth_value == 0:
            return depth_image

        # Create an empty mask for region growing
        region_mask = np.zeros_like(depth_image, dtype=np.uint8)

        # Initialize the region with the seed point
        region_mask[seed_point[1], seed_point[0]] = 1

        # Use a queue to perform the region growing
        region_queue = [seed_point]

        # Neighboring pixel offsets for 8-connectivity (up, down, left, right, diagonals)
        ...
        while region_queue:
            ...
            for dx, dy in neighbors:
                ...
                if 0 <= nx < width and 0 <= ny < height:
                    ...
                    # If the neighbor is valid and not yet part of the region
                    if region_mask[ny, nx] == 0 and neighbor_depth > 0:
                        # Check if the neighbor depth is within the threshold
                        if abs(neighbor_depth - seed_depth_value) <= threshold * seed_depth_value:
                            region_mask[ny, nx] = 1
                            region_queue.append((nx, ny))

        # Apply the region mask to the depth image
        filtered_image = np.where(region_mask == 1, depth_image, 0)
...
def main(args=None):
    ...
    node = YoloSAM2FilterSegmentationNode()
    rclpy.spin(node)
    rclpy.shutdown()
